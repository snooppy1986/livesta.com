            ...
            <div class="row">
                ...
                <div class="col-md-7">
                    <h5 class="showing-pagination-results mt-5 mt-md-9 text-center text-md-end">
                         За запитом "{{search_req}}" знайдено {{count}} товар(ів)
                    </h5>
                </div>
            </div>
        </div>
    ...
    <section class="section-space">
        <div class="container">
                            ...
                            <ul class="product-widget-category">
                                <li v-for="cat in categories">
                                    <router-link :to="'/category/'+cat.id">
                                        {{cat.title}} <span v-if="cat.children && cat.children.length">({{cat.children.length}})</span>
                                    </router-link>
                                </li>
                            </ul>
                        </div>
            ...
            <div v-else class="row g-3 g-sm-6">
                <h3>За запитом "{{search_req}}" нічого не знайдено!!!</h3>
            </div>
        </div>
    </section>
